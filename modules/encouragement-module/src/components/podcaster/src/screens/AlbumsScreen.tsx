...
import { NowPlayingItem, Podcast, createEmailLinkUrl } from 'podverse-shared';
import { Alert, Linking, View as RNView, StyleSheet } from 'react-native';
...
import { GlobalPropertyCallbackFunction, safeKeyExtractor, setCategoryQueryProperty } from '../lib/utility';
...
import {
  getSavedQueryAlbumsScreenFilter,
  getSavedQueryAlbumsScreenSort,
  setSavedQueryAlbumsScreenFilter,
  setSavedQueryAlbumsScreenSort,
...
export class AlbumsScreen extends React.Component<Props, State> {
  ...
  static navigationOptions = ({ navigation, route }) => {
    const _screenTitle = route.params._screenTitle;
    // const _hasSubscribedPodcasts = route.params._hasSubscribedPodcasts
    return {
      title: _screenTitle,
      // headerRight: () =>
    ...
    if (!isInMaintenanceMode) {
      ...
      if (isConnected) {
        ...
        this.setState({ querySort: savedQuerySort }, () => {
          this.handleSelectFilterItem(savedQueryFilter || PV.Filters._allPodcastsKey, keepSearchTitle);
        });
      } else {
    ...
    this.setState(
      ...
      () => {
        (async () => {
          ...
          const options = {};
          const newState = await this._queryData(selectedKey, this.state, nextState, options);
          this.setState({
            ...newState,
  ...
  _onPodcastItemSelected = (podcast: Podcast) => {
    handleAlbumScreenNavigateWithParams(this.props.navigation, podcast.id, podcast, { forceRequest: false });
  };
