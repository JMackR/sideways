...
export class PlaylistsAddToScreen extends React.Component<Props, State> {
  constructor(props: Props) {
    ...
    this.state = {
      ...
      isLoading: true,
      mediaRef: route.params.mediaRef,
    };
  ...
  static navigationOptions = ({ navigation, route }) => {
    const headerLeft = route.params.isModal ? (
        ...
        <RNView>
          {route.params.isLoggedIn && (
            <NavHeaderButtonText
              ...
              handlePress={route.params.showNewPlaylistDialog}
  ...
  render() {
    ...
    const medium = mediaRef?.episode?.podcast?.medium || episode?.podcast?.medium || PV.Medium.mixed;

    // Don't include playlists that are incompatible with the item on
    // the PlaylistsAddToScreen's medium type.
    const finalPlaylists = myPlaylists?.filter(
      (myPlaylist: Playlist) => myPlaylist?.medium === PV.Medium.mixed || myPlaylist?.medium === medium,
    );

    const headerTitle = mediaRef?.title || episode?.title;
    const headerSubtitle = mediaRef?.episode?.podcast?.title || episode?.podcast?.title;
    const headerImage =
      mediaRef?.episode?.imageUrl ||
            ...
            {!isLoading && finalPlaylists && (
              <>
                <View style={styles.headerWrapper}>
                  <FastImage allowFullView source={headerImage} styles={styles.headerImage} />
