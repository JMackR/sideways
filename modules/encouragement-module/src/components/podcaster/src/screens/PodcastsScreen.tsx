...
export class PodcastsScreen extends React.Component<Props, State> {
  ...
  async componentDidMount() {
    ...
    setTimeout(() => {
      console.log('this.props.navigation.setParams', this.props.route.params);
    }, 2000);
    ...
    PVEventEmitter.on(PV.Events.NOTIFICATION_BANNER_PRESSED, (params: { episodeId: string; podcastId: string }) =>
      navigateToEpisodeScreenInPodcastsStackNavigatorWithIds(this.props.navigation, params.podcastId, params.episodeId),
    );
  ...
  setupDeeplinkListeners = () => {
    if (checkIfFDroidAppVersion() && !!this.pvNativeEventEmitter) {
      ...
      messaging().onMessage((notification) => {
        ...
        const description = notificationType === 'live' ? notificationTitle : episodeTitle;
  ...
  _initializeScreenData = async () => {
    ...
    const initalPodcasts = initialPodcastsAllMediums.filter(
  ...
  _initializeScreenDataPart2 = async () => {
    ...
    await Promise.all([initAutoQueue(), initializePlayer(), initializePlayerSettings()]);
  ...
  _onPodcastItemSelected = (podcast: Podcast) => {
    ...
    if (isMusic) {
      ...
      handlePodcastScreenNavigateWithParams(navigation, podcast.id, podcast, { forceRequest: false });
    }
  };
