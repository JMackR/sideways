import { NavigationStackOptions } from '@react-navigation/native-stack';
import {
...
export class AlbumScreen extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    const podcastId = podcast?.id || podcast?.addByRSSPodcastFeedUrl || props.route.params.podcastId;
  ...
  static navigationOptions = ({ navigation, route }) => {
    ...
    const podcastTitle = route.params.podcastTitle;
    const podcast = route.params.podcast;
    const addByRSSPodcastFeedUrl = route.params.addByRSSPodcastFeedUrl;
    const notificationsEnabled = route.params.notificationsEnabled;
  ...
  getDefaultFilters = (props: Props) => {
    ...
    const hasInternetConnection = route.params.hasInternetConnection;
    const { isInMaintenanceMode } = this.global;
    let viewType = PV.Filters._tracksKey;
    const viewTypeOverride = route.params.viewType;
  ...
  async componentDidMount() {
    ...
    let podcast = route.params.podcast;
    const forceRequest = route.params.forceRequest;
  ...
  _initializePageData() {
    ...
    const podcastId = route.params.podcastId || this.state.podcastId;
  ...
  _renderItem = ({ item, index }) => {
    ...
    return (
      <TrackTableCell
        episode={episode}
        handleMorePress={() => this._handleMorePress(newNowPlayingItem)}
  ...
  _getFinalFeedUrl = () => {
    ...
    return this.props.route.params.addByRSSPodcastFeedUrl || getAuthorityFeedUrlFromArray(feedUrlObjects);
  };
  ...
  _queryData = async (filterKey: string | null, queryOptions: { queryPage?: number } = {}) => {
    ...
    try {
      if (
        (filterKey === PV.Filters._downloadedKey || filterKey === PV.Filters._tracksKey) &&
        podcast &&
