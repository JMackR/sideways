...
import {
  ...
  sortPodcastArrayAlphabetically,
  subscribeToPodcastIfNotAlready,
...
const handleAddOrRemoveByRSSPodcast = async (
  feedUrl: string,
  shouldAdd: boolean,
  credentials?: string,
  throwError?: boolean,
...
const addAddByRSSPodcastIfNotAlready = async (
  alreadySubscribedAddByRSSPodcasts: any[],
  feedUrl: string,
  skipBadParse?: boolean,
) => {
  if (
    Array.isArray(alreadySubscribedAddByRSSPodcasts) &&
    !alreadySubscribedAddByRSSPodcasts.some(
      (alreadySubscribedAddByRSSPodcast) => alreadySubscribedAddByRSSPodcast.addByRSSPodcastFeedUrl === feedUrl,
    )
  ) {
