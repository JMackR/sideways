...
import {
  ...
  setChapterOnGlobalState,
} from './playerChapters';
import { videoInitializePlayer, videoStateClearVideoInfo, videoStateSetVideoInfo } from './playerVideo';
...
const clearEnrichedPodcastDataIfNewEpisode = async (
...
export const playerLoadNowPlayingItem = async (item: NowPlayingItem, options: PlayerLoadNowPlayingItemOptions) => {
  ...
  if (!!item?.episodeId) {
    ...
    await playerLoadNowPlayingItemService(item, {
      shouldPlay,
