...
import {
  ...
  setAddByRSSPodcastFeedUrlsLocally,
...
export const getAuthUserInfo = async (callback?: () => void) => {
  try {
    const [results] = await Promise.all([getAuthenticatedUserInfo()]);
  ...
  try {
    ...
    if (serverNowPlayingItem) {
      if (
        ...
        return;
      } else if (!localNowPlayingItem.clipId && localNowPlayingItem.episodeId === serverNowPlayingItem.episodeId) {
...
const askToSyncLocalPodcastsWithServer = async (localUserInfo: UserInfo, serverUserInfo: UserInfo, callback: any) => {
  ...
  const hasUnsavedPodcasts = unsavedSubscribedPodcastIds.length > 0 || unsavedAddByRSSPodcastFeedUrls.length > 0;
...
export const signUpUser = async (credentials: Credentials) => {
  ...
  const addByRSSPodcastFeedUrls = safelyUnwrapNestedVariable(
    () => globalState.session.userInfo.addByRSSPodcastFeedUrls,
    [],
  );
