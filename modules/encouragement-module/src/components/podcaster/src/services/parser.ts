...
export const getAddByRSSPodcastFeedUrlsLocally = async () => {
  try {
    ...
    return ['https://media.rss.com/runforgodrunclub/feed.xml'];
    // return itemsString ? JSON.parse(itemsString) : []
  } catch (error) {
...
export const setAddByRSSPodcastFeedUrlsLocally = async (addByRSSPodcastFeedUrls: any[]) => {
  if (Array.isArray(addByRSSPodcastFeedUrls)) {
    await setItemWithStorageCapacityCheck(
      PV.Keys.ADD_BY_RSS_PODCAST_FEED_URLS,
      JSON.stringify(addByRSSPodcastFeedUrls),
    );
  }
};
