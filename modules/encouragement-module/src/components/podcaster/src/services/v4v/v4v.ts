...
import {
  ...
  NowPlayingItem,
  ValueRecipient,
  ValueRecipientNormalized,
  ValueTag,
  ValueTimeSplit,
  ValueTransaction,
  checkIfIsLightningKeysendValueTag,
...
export const convertValueTagIntoValueTransactions = async (
  ...
  for (const parentValueTagFeeRecipient of parentValueTagFeeRecipients) {
    const parentValueTagFeeRecipientValueTransaction = await convertFeeRecipientsIntoValueTransaction(
      parentValueTagFeeRecipient,
    );
  ...
  const normalizedNonFeeValueRecipients = normalizeValueRecipients(
    nonFeeRecipients,
    nonFeeRecipientsAmount,
    roundDownValues,
  );

  const normalizedParentNonFeeValueRecipients = normalizeValueRecipients(
    parentValueTagNonFeeRecipients,
    parentValueTagNonFeeRecipientsAmount,
    roundDownValues,
  );
  ...
  try {
    if (valueTransactions?.length > 0) {
      ...
      for (const valueTransaction of valueTransactions) {
        ...
        try {
          const podcastIndexData = await getEpisodeByGuidFromPodcastIndex(podcastIndexId, episodeGuid);
      ...
      const response = await sendValueTransactions(finalValueTransactions, 'alby', boostagramMessage);
      ...
      if (keysendsData) {
        for (const keysendData of keysendsData) {
          if (keysendData?.error) {
            processSendValueTransactionError(keysendData, customKeyValueAddresses, type);
          } else {
    ...
    if (hasErrorResponseData && failedKeysends.length > 0) {
      for (const failedKeysend of failedKeysends) {
        if (failedKeysend?.error?.error) {
          processSendValueTransactionError(failedKeysend, error?.response?.data?.customKeyValueAddresses || [], type);
        }
      }
    }
  }
...
export const sendBoost = async (
  ...
  const action = 'boost';
  const valueTags =
    (item?.episodeValue?.length && item?.episodeValue) || (item?.podcastValue?.length && item?.podcastValue);
  ...
  const { activeProvider, activeProviderSettings } = v4vGetActiveProviderInfo(getBoostagramItemValueTags(item));
  ...
  const { feeValueTransactions, nonFeeValueTransactions, parentFeeValueTransactions, parentNonFeeValueTransactions } =
...
const sendValueTransactions = async (
  ...
  if (providerKey) {
    ...
    if (providerKey === 'alby') {
      ...
      response = await v4vAlbySendKeysendPayments(filteredTransactions, boostagramMessage);
    }
  }
...
export const extractV4VValueTags = (episodeValue?: ValueTag[], podcastValue?: ValueTag[]) => {
  return (episodeValue?.length && episodeValue) || (podcastValue?.length && podcastValue) || [];
};
