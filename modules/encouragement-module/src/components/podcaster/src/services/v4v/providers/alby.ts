...
const _fileName = 'src/services/v4v/providersalby.ts';
...
export const v4vAlbySendKeysendPayments = async (transactions: ValueTransaction[], boostagramMessage?: string) => {
  ...
  for (const transaction of transactions) {
    ...
    if (transaction.normalizedValueRecipient.customKey && transaction.normalizedValueRecipient.customValue) {
