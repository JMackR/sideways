...
import {
  ...
  getHistoryItemIndexInfoForEpisode,
    ...
    if (nowPlayingItem) {
      ...
      if (episodeId) {
        if (secondaryQueuePlaylistId) {
          ...
          nextNowPlayingItems = nextEpisodesAndMediaRefs.map((nextEpisodeOrMediaRef: Episode | MediaRef) => {
          ...
          if (!isMusic && autoPlayEpisodesFromPodcast === 'off') {
            ...
            const sortedPreviousEpisodes =
              !isMusic && autoPlayEpisodesFromPodcast === 'older' ? nextEpisodes?.reverse() : previousEpisodes;

            const sortedNextEpisodes =
              !isMusic && autoPlayEpisodesFromPodcast === 'older' ? previousEpisodes?.reverse() : nextEpisodes;

            previousNowPlayingItems = sortedPreviousEpisodes.map((previousEpisode: Episode) => {
              const userPlaybackPosition =
                inheritedPodcast?.medium === PV.Medium.music
                  ? 0
                  : getHistoryItemIndexInfoForEpisode(previousEpisode?.id)?.userPlaybackPosition || 0;
