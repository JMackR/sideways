...
export class PVFastImage extends React.PureComponent<Props, State> {
  ...
  checkIfVTSIsInDefaultPlaylist = () => {
    ...
    const localDefaultPlaylist = myPlaylists.find(
      (playlist: any) => playlist.isDefault && playlist.medium === remoteMedium,
    );
    let isInDefaultPublic = false;
    if (localDefaultPlaylist) {
      isInDefaultPublic = localDefaultPlaylist.itemsOrder?.some(
        (itemId: string) => itemId === currentChapter?.remoteEpisodeId,
      );
    }
  ...
  handleLikePress = async () => {
    ...
    if (isInDefaultPlaylist && currentChapter?.remoteEpisodeId) {
      ...
      await addOrRemovePlaylistItemToDefaultPlaylist(currentChapter.remoteEpisodeId, '', navigation);
    }
  };
