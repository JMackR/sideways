...
export const DropdownBanner = ({
  ...
  const GestureContentWrapper = gestureHandlerRootHOC(() => (
    <GestureDetector gesture={Gesture.Fling().direction(Directions.DOWN).onStart(expandBanner)}>
      <GestureDetector gesture={Gesture.Fling().direction(Directions.UP).onStart(closeBanner)}>
        <View>
          {children}
