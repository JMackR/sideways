...
export class TrackTableCell extends React.PureComponent<Props> {
  render() {
    const { episode, handleMorePress, handlePlayPress, hideImage = true, showArtist, testID } = this.props;

    const { downloadedEpisodeIds, downloadsActive, player, screenReaderEnabled } = this.global;
    ...
    const textWrapperStyle = showArtist ? [styles.textWrapper] : [styles.textWrapper, { justifyContent: 'center' }];

    const textStyle = showArtist ? [styles.text, { marginTop: 1 }] : [styles.text];
      ...
      <RNView style={styles.outerWrapper}>
        ...
        >
          ...
          <RNView style={textWrapperStyle}>
            <Text numberOfLines={1} style={[textStyle, activeTrackStyle]}>
              {trackTitle}
            </Text>
            {showArtist && (
          ...
          {!!episodeDownloaded && <IndicatorDownload style={styles.autoDownloadIcon} />}
        </Pressable>
