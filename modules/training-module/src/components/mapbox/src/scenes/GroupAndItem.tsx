...
class ExampleGroup implements ExampleNode {
  ...
  find(path: string[]): ExampleNode | undefined {
    ...
    if (root === this.label) {
      if (rest.length > 0) {
        return this.items.reduce<ExampleNode | undefined>((prev, act) => prev || act.find(rest), undefined);
      } else {
        return this;
      }
    } else {
...
const PageWrapper = (Component: ItemComponent) => (props: BaseExampleProps) =>
  (
    <Page label={props.label} onDismissExample={props.onDismissExample} navigation={props.navigation}>
      <Component {...props} />
    </Page>
  );
...
export const Group = ({ route, navigation }: GroupProps) => {
  ...
  const { items } = item;
  return <ExampleGroupComponent items={items} navigation={navigation} showBack={path.length > 1} title={item.label} />;
};
...
export const Item = ({ route, navigation }: ItemProps) => {
  ...
  useEffect(() => {
    AsyncStorage.setItem(MostRecentExampleKey, JSON.stringify(route.params.path));
  }, [route.params.path]);
  const { path } = route.params;
  const item = Examples.find(path);
  if (!(item instanceof ExampleItem)) {
    throw Error(`error:Expected item not group|undefined! path:${path} item:${item}`);
  }
  const { label, Component } = item;

  return <Component label={label} onDismissExample={onDismissExample} navigation={navigation} />;
};
