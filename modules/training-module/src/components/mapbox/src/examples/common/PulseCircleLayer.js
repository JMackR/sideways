...
class PulseCircleLayer extends React.Component {
  ...
  componentDidMount() {
    ...
    this._loopAnim = Animated.loop(Animated.sequence([expandOutAnim, shrinkInAnim]));
  ...
  render() {
    ...
    const innerCircleStyle = [styles.innerCircle, this.props.innerCircleStyle, { circleRadius: this.props.radius }];

    const innerCirclePulseStyle = [styles.innerCirclePulse, { circleRadius: this.state.innerRadius }];
