...
const SettingsGroup = ({ children, label }: { children: ReactNode; label: string }) => (
  <View>
...
const ANDROID_RENDER_MODES: ('normal' | 'compass' | 'gps')[] = ['normal', 'compass', 'gps'];

const UserLocationRenderMode = () => {
  const [renderMode, setRenderMode] = useState<ExampleRenderMode>(ExampleRenderMode.Normal);
  ...
  const [androidRenderMode, setAndroidRenderMode] = useState<'normal' | 'compass' | 'gps'>('normal');
