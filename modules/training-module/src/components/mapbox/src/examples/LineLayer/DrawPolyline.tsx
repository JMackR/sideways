...
import React, { useState, useRef, ComponentProps, useMemo, forwardRef } from 'react';
...
const Crosshair = forwardRef<View, CrosshairProps>(({ size, w, onLayout }: CrosshairProps, ref) => (
  <View
    onLayout={onLayout}
    ref={ref}
    style={{
...
const CrosshairOverlay = ({ onCenter }: { onCenter: (x: [number, number]) => void }) => {
    ...
    <View style={{ flex: 1 }}>
      <View>
        {!started ? (
          ...
          >
            <Button title="add" onPress={() => setCoordinates([...coordinates, lastCoordinate])} />
      ...
      <View style={{ flex: 1 }}>
        <MapView
          ...
          onCameraChanged={async (e) => {
            const crosshairCoords = await map.current?.getCoordinateFromView(crosshairPos);
            console.log('Crosshair coords: ', crosshairCoords, 'camera center:', e.properties.center);
