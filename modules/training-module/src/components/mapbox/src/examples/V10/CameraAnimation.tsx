...
import { Camera, CameraAnimationMode, CameraBounds, CircleLayer, Logger, MapView, ShapeSource } from '@rnmapbox/maps';
...
const CameraAnimation = () => {
  ...
  const [coordinates, setCoordinates] = useState<Coordinate[]>([initialCoordinate]);
    ...
    return (
      <View style={{ flex: 1, paddingHorizontal: 10 }}>
        <View style={{ flex: 0, alignItems: 'center' }}>
          <Text style={{ fontWeight: 'bold', opacity: disabled ? 0.4 : 1 }}>{zoom}</Text>
        </View>
        <Slider
          thumbStyle={[styles.thumb, { backgroundColor: disabled ? 'lightgray' : 'black' }]}
  ...
  const paddingCounter = useCallback((value: number, setValue: (value: number) => void, label: string) => {
    return (
      <View style={{ flex: 1, paddingHorizontal: 10 }}>
        <View style={{ flex: 0, alignItems: 'center' }}>
          <Text>{label}</Text>
  ...
  const zoomLimitCounter = useCallback(
    (value: number | undefined, setValue: (value?: number) => void, label: string) => {
      return (
        <View style={{ flex: 1, paddingHorizontal: 10 }}>
          <View style={{ flex: 0, alignItems: 'center' }}>
            <Text>{label}</Text>
            <Text style={{ fontWeight: 'bold' }}>{`${value ?? 'Not set'}`}</Text>
          </View>
          <Slider
            thumbStyle={styles.thumb}
            value={value}
      ...
      <SafeAreaView>
        <View style={styles.sheet}>
          <View style={styles.content}>
            ...
            <View style={[styles.buttonRow, { marginBottom: -6 }]}>{zoomCounter}</View>
