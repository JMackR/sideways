...
import { Camera, CircleLayer, Logger, MapState, MapView, ShapeSource } from '@rnmapbox/maps';
...
const MapHandlers = () => {
  ...
  return (
    <>
      ...
      >
        <Camera centerCoordinate={[-73.984638, 40.759211]} zoomLevel={12} animationDuration={0} />
        {features.map((f, i) => {
          const id = JSON.stringify((f as Feature<Polygon>).geometry.coordinates);
          const circleStyle =
      ...
      <SafeAreaView>
        <View style={styles.info}>
          <Text style={styles.fadedText}>Tap or long-press to create a marker.</Text>
