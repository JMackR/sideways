...
const Markers = () => {
  ...
  const randomizeCoordinatesAndColors = useCallback(() => {
    const newMarkers = new Array(markerCount).fill(0).map((o, i) => {
      return {
        coords: [centerCoord[0] + (Math.random() - 0.5) * 0.008, centerCoord[1] + (Math.random() - 0.5) * 0.008],
  ...
  return (
    <>
      <MapView style={{ flex: 1 }}>
        ...
        {markers.map((marker, i) => {
          return (
            ...
            >
              <Pressable
                style={[styles.markerBox, { backgroundColor: marker.color, padding: 4 * size }]}
                onPress={() => setSelectedIndex((index) => (index === i ? -1 : i))}
              >
      ...
      <View style={styles.buttonsHolder}>
        <Button style={styles.button} title={'Rearrange'} onPress={randomizeCoordinatesAndColors} />
