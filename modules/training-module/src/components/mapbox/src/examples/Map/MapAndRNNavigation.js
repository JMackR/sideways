...
export default function MapAndNavigation({ navigation }) {
  ...
  return (
    <>
      ...
      <Button title="Toggle map" onPress={() => setShowMap((showMap) => !showMap)} />
      <Button title="Navigate" onPress={() => navigation.navigate('ScreenWithoutMap')} />
      <Modal
        animationType="slide"
        transparent={false}
      ...
      {showMap && (
        <MapView style={{ flex: 1 }}>
          <Camera centerCoordinate={[-74.00597, 40.71427]} zoomLevel={14} followUserLocation followZoomLevel={14} />
          <UserLocation androidRenderMode={'gps'} showsUserHeadingIndicator={true} renderMode={'native'} />
        </MapView>
      )}
    </>
  );
