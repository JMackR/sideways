...
class QueryWithRect extends React.Component {
  ...
  async onPress(e) {
    ...
    if (screenCoords.length === 2) {
      const featureCollection = await this._map.queryRenderedFeaturesInRect(this.getBoundingBox(screenCoords), null, [
        'nycFill',
      ]);
  ...
  render() {
    return (
      <Page {...this.props}>
        ...
        >
          <MapboxGL.Camera zoomLevel={9} centerCoordinate={[-73.970895, 40.723279]} />
          ...
          {this.state.selectedGeoJSON ? (
            <MapboxGL.ShapeSource id="selectedNYC" shape={this.state.selectedGeoJSON}>
              <MapboxGL.FillLayer id="selectedNYCFill" style={styles.selectedNeighborhoods} />
            </MapboxGL.ShapeSource>
          ) : null}
        </MapboxGL.MapView>
