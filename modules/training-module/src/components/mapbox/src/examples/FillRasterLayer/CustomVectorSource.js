...
const styles = {
  boxFill: {
    fillColor: ['interpolate', ['linear'], ['get', 'box'], 0, 'green', 1, 'blue'],
...
class CustomVectorSource extends React.PureComponent {
  ...
  queryFeatures = async () => {
    const features = await this._vectorSource.features(['react-native-example']);
  ...
  render() {
    return (
      <MapView style={styles.matchParent}>
        ...
        >
          <FillLayer id="customSourceFill" sourceLayerID="react-native-example" style={styles.boxFill} />
        </VectorSource>
      </MapView>
    );
  }
