...
import { StyleURL, MapView, FillLayer, VectorSource, Camera } from '@rnmapbox/maps';
    ...
    try {
      ...
      setMessage(
        `[SUCCESS] querySourceFeatures retrieved ${featuresCount} features ` +
          `and includes: \n- ${res.features.map((f) => f.properties?.COUNTY).join('\n- ')}`,
      );
    } catch (err) {
      console.error(err);
    ...
    <>
      ...
      >
        <VectorSource {...vectorSourceUnderTest}>
          <FillLayer sourceID={'counties'} sourceLayerID="original" id="counties" belowLayerID="building" />
        </VectorSource>
        <Camera
