...
class DriveTheLine extends React.Component {
  ...
  async componentDidMount() {
    const reqOptions = {
      waypoints: [{ coordinates: SF_OFFICE_COORDINATE }, { coordinates: SF_ZOO_COORDINATE }],
      profile: 'walking',
      geometries: 'geojson',
    };
  ...
  renderCurrentPoint() {
    ...
    return <PulseCircleLayer shape={this.state.currentPoint} aboveLayerID="destinationInnerCircle" />;
  }
  ...
  renderProgressLine() {
    ...
    console.log('currentpoint', this.state.currentPoint);
  ...
  renderActions() {
    ...
    return (
      <View style={styles.buttonCnt}>
        <Button raised title="Start" onPress={this.onStart} style={styles.button} disabled={!this.state.route} />
      </View>
    );
  }
  ...
  render() {
    ...
    return (
      <Page {...this.props}>
        <MapboxGL.MapView ref={(c) => (this._map = c)} style={sheet.matchParent} styleURL={MapboxGL.StyleURL.Dark}>
          <MapboxGL.Camera zoomLevel={11} centerCoordinate={[-122.452652, 37.762963]} />
          ...
          <MapboxGL.ShapeSource id="destination" shape={point(SF_ZOO_COORDINATE)}>
