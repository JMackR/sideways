...
const AnnotationWithRemoteImage = ({ id, coordinate, title }: AnnotationWithRemoteImageProps) => {
...
const ShowPointAnnotation = () => {
  ...
  const [layerRendering, setLayerRendering] = useState<'below' | 'above'>('below');
  ...
  const renderAnnotations = () => {
    ...
    for (let i = 0; i < coordinates.length; i++) {
      ...
      if (i % 2 === 1) {
        items.push(null, <AnnotationWithRemoteImage key={id} id={id} coordinate={coordinate} title={title} />);
      } else {
        items.push(
          null,
          <PointAnnotation key={id} id={id} coordinate={coordinate} title={title}>
            <View style={styles.annotationContainer} />
  ...
  return (
    <>
      <MapView
        onPress={(feature) => {
          setCoordinates((prevState) => [...prevState, (feature.geometry as Point).coordinates]);
        }}
        style={styles.matchParent}
        deselectAnnotationOnTap={true}
      >
        <Camera defaultSettings={{ centerCoordinate: coordinates[0], zoomLevel: 16 }} />
        ...
        >
          <FillLayer
            id="polygon"
            {...{
              [layerRendering + 'LayerID']: getAnnotationsLayerID('PointAnnotations'),
            }}
            style={{
      ...
      <Bubble>
        ...
        <Button
          onPress={() => setLayerRendering((prevState) => (({ above: 'below', below: 'above' } as const)[prevState]))}
        >
