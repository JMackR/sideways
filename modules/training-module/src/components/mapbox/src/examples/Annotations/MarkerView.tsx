...
const ShowMarkerView = () => {
  ...
  return (
    <>
      <Button
        title={allowOverlapWithPuck ? 'allowOverlapWithPuck true' : 'allowOverlapWithPuck false'}
      ...
      <Mapbox.MapView onPress={onPressMap} style={styles.matchParent}>
        ...
        <Mapbox.MarkerView coordinate={pointList[0]} allowOverlapWithPuck={allowOverlapWithPuck}>
          <AnnotationContent title={'this is a marker view'} />
        </Mapbox.MarkerView>

        {pointList.slice(2).map((coordinate, index) => (
          <Mapbox.PointAnnotation coordinate={coordinate} id={`pt-ann-${index}`} key={`pt-ann-${index}`}>
            <AnnotationContent title={'this is a point annotation'} />
          </Mapbox.PointAnnotation>
        ))}
