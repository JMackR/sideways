...
class Fit extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    const changed = (stateKey) => {
      ...
      return !isEqual(prevState[stateKey], this.state[stateKey]) && this.state[stateKey] !== undefined;
    };
  ...
  render() {
    const { locationType, zoomLevel, followUserLocation, padding, cachedFlyTo, cachedZoomLevel } = this.state;
    ...
    return (
      <>
        <Mapbox.MapView styleURL={Mapbox.StyleURL.Satellite} style={sheet.matchParent}>
          <Mapbox.Camera ref={(ref) => (this.camera = ref)} {...this.cameraProps()} />
