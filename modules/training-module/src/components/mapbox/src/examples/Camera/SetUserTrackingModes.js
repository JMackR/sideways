      ...
      <>
        <ScrollView horizontal style={{ maxHeight: 64 }}>
          <ButtonGroup
            ...
            onPress={(i) => this.onTrackingChange(i, this._trackingOptions[i].data)}
          />
        </ScrollView>
        <Mapbox.MapView style={styles.matchParent}>
          ...
          <Mapbox.Camera
            ...
            followUserLocation={this.state.userSelectedUserTrackingMode !== 'none'}
            followUserMode={
              this.state.userSelectedUserTrackingMode !== 'none' ? this.state.userSelectedUserTrackingMode : 'normal'
            }
        ...
        <Bubble onPress={this.onToggleUserLocation} style={styles.bubbleTwo}>
          <Text>Toggle User Location: {this.state.showUserLocation ? 'true' : 'false'}</Text>
        </Bubble>

        <Bubble onPress={this.onToggleHeadingIndicator} style={styles.bubbleThree}>
          <Text>Toggle user heading indicator: {this.state.showsUserHeadingIndicator ? 'true' : 'false'}</Text>
        </Bubble>
      </>
    );
  }
