...
const styles = {
  circles: {
    circleRadius: ['interpolate', ['exponential', 1.75], ['zoom'], 12, 2, 22, 180],
...
class DataDrivenCircleColors extends React.PureComponent {
  render() {
    return (
      <>
        <Mapbox.MapView styleURL={Mapbox.StyleURL.Light} style={styles.matchParent}>
          <Mapbox.Camera
            defaultSettings={{
          ...
          <Mapbox.VectorSource id="population" url={'mapbox://examples.8fgz4egr'}>
            <Mapbox.CircleLayer id="sf2010CircleFill" sourceLayerID="sf2010" style={styles.circles} />
          </Mapbox.VectorSource>
        </Mapbox.MapView>
      </>
    );
