...
import MapboxGL, { MapView, Camera, ShapeSource, SymbolLayer, Images } from '@rnmapbox/maps';
...
const CustomIconNativeAsset = memo(() => {
  ...
  const [stateFeatureCollection, setStateFeatureCollection] = useState<GeoJSON.FeatureCollection>(
  ...
  const onPress = (e: GeoJSON.Feature) => {
    ...
    setStateFeatureCollection(featureCollection([...stateFeatureCollection.features, aFeature]));
  };

  const onSourceLayerPress = (e: any) => {
    console.log('You pressed a layer here are your features:', e.features, e.coordinates, e.point);
  };

  return (
    <>
      <MapView style={{ flex: 1 }} styleURL={MapboxGL.StyleURL.Light} onPress={onPress}>
        <Camera
          ref={cameraRef}
          defaultSettings={{
            zoomLevel: 9,
        ...
        >
          <SymbolLayer id="symbolLocationSymbols" minZoomLevel={1} style={styles.icon} />
