    ...
    <Wrapper navigation={navigation} title={'Statistics'}>
      <ScrollView showsVerticalScrollIndicator={false} style={{ paddingHorizontal: '3%' }}>
        <View style={styles.root}>
          <Image style={styles.story} source={require('../resources/img/stats.png')} />
          ...
          <>
            ...
            {dataLoaded && bestMarkers.length > 0 && bestActiv && (
              ...
              >
                ...
                <MapView
                  ...
                  initialRegion={{
                    latitude:
                      (Math.max(...bestMarkers.map((p: Point) => p.latitude)) +
                        Math.min(...bestMarkers.map((p: Point) => p.latitude))) /
                      2,
                    longitude:
                      (Math.max(...bestMarkers.map((p: Point) => p.longitude)) +
                        Math.min(...bestMarkers.map((p: Point) => p.longitude))) /
                      2,
                    latitudeDelta:
                      (Math.max(...bestMarkers.map((p: Point) => p.latitude)) -
                        Math.min(...bestMarkers.map((p: Point) => p.latitude))) *
                ...
                <View style={[styles.window_section, { flexDirection: 'row' }]}>
                  <View style={styles.column}>
                    ...
                    {bestActiv.distance < 1000 && (
                      <Text style={styles.column_text}>{bestActiv.distance.toFixed(2)} m</Text>
                    )}

                    {bestActiv.distance >= 1000 && (
                      <Text style={styles.column_text}>{(bestActiv.distance / 1000).toFixed(2)} km</Text>
                    )}
                  </View>
                  ...
                  <View style={styles.column}>
                    ...
                    <Text style={styles.column_text}>{speed(bestActiv).toFixed(2)} km/h</Text>
                  </View>
                </View>
              </TouchableOpacity>
            )}
            ...
            <View style={styles.section}>
              ...
              <View style={styles.span}>
                ...
                <Text style={styles.result}>{bestTime(times)}</Text>
              </View>

              <View style={styles.span}>
                <Text style={styles.info}>Average time: </Text>
                <Text style={styles.result}>{meanTime(times)}</Text>
              </View>
            </View>
            ...
            <View style={styles.section}>
              <View style={styles.span}>
                ...
                <Text style={styles.result}>{(sumDistance / 1000).toFixed(2)} km</Text>
              </View>
              ...
              <View style={styles.span}>
                ...
                <Text style={styles.result}>{(sumDistance / 1000 / activities.length).toFixed(2)} km</Text>
              </View>
            </View>
