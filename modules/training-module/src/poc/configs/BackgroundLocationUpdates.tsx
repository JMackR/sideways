...
export default function BackgroundLocationUpdates() {
      ...
      if (nextAppState.match(/inactive|background/)) {
        BackgroundTimer.runBackgroundTimer(() => {
          Geolocation.getCurrentPosition(
            (position) => {
              console.log('getCurrentPosition background', JSON.stringify(position));
            },
            (error) => console.log('getCurrentPosition background error', JSON.stringify(error)),
  ...
  return (
    <View>
      <Button
        title={`${backgroundListener ? 'Disable' : 'Enable'} background location updates`}
