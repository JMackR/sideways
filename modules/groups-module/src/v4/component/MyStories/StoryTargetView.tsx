...
const StoryTargetView: FC<IStorytargetView> = ({
  ...
  const onFinish = useCallback(
    (state?: NextOrPrevious) => {
      ...
      if (state === 'previous') {
        if (currentCommunityIndex > 0) return setCurrentCommunityIndex(currentCommunityIndex - 1);
      }
      setViewStory(false);
      onClose && onClose();
    },
    [currentCommunityIndex, globalStoryTargets.length, onClose, setCurrentCommunityIndex, setViewStory],
  );
