...
import { officialIcon, privateIcon, storyRing } from '../../../svg/svg-xml-list';
...
const StoryCircleItem: FC<IStoryCircleItem> = ({ onPressStoryView, storyTarget }) => {
  ...
  useEffect(() => {
    ...
    CommunityRepository.getCommunity(storyTarget.targetId, async ({ error, loading, data }) => {
      if (error) return;
      if (!loading) {
  ...
  return (
    ...
    >
      ...
      />
      <SvgXml style={styles.storyRing} width={68} height={68} xml={storyRing(storyRingColor[0], storyRingColor[1])} />
      {communityData?.isOfficial && <SvgXml style={styles.officialIcon} xml={officialIcon(theme.colors.primary)} />}
      <View style={styles.textRow}>
        {!communityData?.isPublic && <SvgXml width={17} height={17} xml={privateIcon(theme.colors.base)} />}
