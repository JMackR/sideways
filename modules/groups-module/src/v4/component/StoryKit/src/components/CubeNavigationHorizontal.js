...
export default class CubeNavigationHorizontal extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    const onDoneSwiping = (gestureState) => {
      ...
      setTimeout(() => {
        ...
        if (this.props.callBackAfterSwipe) this.props.callBackAfterSwipe(currentPage);
      }, 500);
    };
  ...
  _renderChild = (child, i) => {
    ...
    return (
      <Animated.View
        style={[StyleSheet.absoluteFill, { backgroundColor: 'transparent' }, this._getTransformsFor(i, false)]}
        key={`child- ${i}`}
