...
import { storyCircleCreatePlusIcon, storyRing } from '../../../svg/svg-xml-list';
...
const CommunityStories = ({ communityId, displayName, avatarFileId }: ICommunityStories) => {
  ...
  const renderCommunityStory = () => {
    if (storyTarget?.lastStoryExpiresAt) {
      return (
        <TouchableOpacity style={styles.avatarContainer} onPress={onPressStoryView}>
          <Image
            source={
              avatarUrl
                ? {
          ...
          />
          {hasStoryPermission && <SvgXml style={styles.storyCreateIcon} xml={storyCircleCreatePlusIcon()} />}
        </TouchableOpacity>
      );
    }
    if (hasStoryPermission) {
      return (
        <TouchableOpacity style={styles.avatarContainer} onPress={onPressCreateStory}>
          <Image
            source={
              avatarUrl
                ? {
            ...
            style={styles.communityAvatar}
          />
          <SvgXml style={styles.storyRing} width={48} height={48} xml={storyRing('#EBECEF', '#EBECEF')} />
          <SvgXml style={styles.storyCreateIcon} xml={storyCircleCreatePlusIcon()} />
        </TouchableOpacity>
      );
    }
    return null;
