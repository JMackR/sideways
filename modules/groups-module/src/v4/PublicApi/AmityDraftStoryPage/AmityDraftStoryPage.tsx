...
const AmityDraftStoryPage: FC<IAmityDraftStoryPage> = ({
  ...
  const imageDisplayMode: Amity.ImageDisplayMode = isFullScreen ? 'fill' : 'fit';
  ...
  useEffect(() => {
    ...
    CommunityRepository.getCommunity(targetId, async ({ error, loading, data }) => {
      if (error) return;
      if (!loading) {
  ...
  const onHyperLinkSubmit = useCallback((item?: { url: string; customText: string }) => {
    if (!item) {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.imageContainer}>
        {type === StoryType.image ? (
          <Image source={{ uri: mediaType.uri }} style={[styles.image, isFullScreen && styles.aspect_ratio]} />
        ) : (
          <Video
            paused={loading}
            repeat
      ...
      {hyperlink?.length > 0 && (
        <TouchableOpacity style={styles.hyperlinkContainer} onPress={onPressHyperLink}>
          ...
          <Text style={styles.hyperlinkText}>
            {hyperlink[0].data.customText.length === 0 ? hyperlink[0].data.url : hyperlink[0].data.customText}
          </Text>
        </TouchableOpacity>
      )}
      <TouchableOpacity style={styles.shareStoryBtn} onPress={onPressShareStory}>
        <Image source={{ uri: communityAvatarUrl }} style={styles.avatar} />
        <Text style={styles.shareStoryTxt}>Share story</Text>
        <SvgXml xml={rightLongArrow(theme.colors.baseShade2)} width={16} height={16} />
      </TouchableOpacity>
      <TouchableOpacity style={styles.backBtn} onPress={onPressBack}>
        <SvgXml xml={leftLongArrow('white')} width={32} height={32} />
      </TouchableOpacity>
      <TouchableOpacity style={styles.aspectRatioBtn} onPress={onPressAspectRatio}>
