...
const HyperlinkConfig: FC<IHyperLinkConfig> = ({
  ...
  const [customizedText, setCustomizedText] = useState(hyperlinkItem?.customText ?? '');
  ...
  const validateHyperlinkUrl = useCallback(async () => {
    if (url.length > 0) {
      const validatedUrl = await (client as Amity.Client).validateUrls([url]).catch(() => false);
  ...
  const validateHyperlinkText = useCallback(async () => {
    if (customizedText.length > 0) {
      const validatedCustomText = await (client as Amity.Client).validateTexts([customizedText]).catch(() => false);
  ...
  const validateHyperlink = useCallback(async () => {
    ...
    validUrl && validText && onHyperLinkSubmit({ url, customText: customizedText });
  ...
  return (
    <Modal
      ...
      coverScreen={true}
    >
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.commentBottomSheet}>
        ...
        {loading && <ActivityIndicator animating={loading} size="small" color={theme.colors.base} />}
        <View style={[styles.inputContainer, url.length > 0 && urlErrorMessage && styles.alertBorderColor]}>
        ...
        {urlErrorMessage && <Text style={styles.inValidUrl}>{urlErrorMessage}</Text>}

        <View
          style={[
            styles.inputContainer,
            customizedText.length > 0 && customTextErrorMessage && styles.alertBorderColor,
          ]}
        >
        ...
        {customTextErrorMessage ? (
          ...
          <Text style={styles.note}>This Text will show on the link instead of URL.</Text>
        )}
        {hyperlinkItem && (
          <View style={styles.inputContainer}>
            <TouchableOpacity style={styles.deleteHyperlinkContainer} onPress={onPressDeleteHyperlink}>
