...
const AmityViewStoryItem: FC<IAmityViewStoryItem> = ({
  ...
  }, [communityData?.isJoined, currentStory?.storyId, handleReaction, isLiked, progress, startAnimation]);
  ...
  const deleteStory = useCallback(async () => {
    ...
    try {
      current > 0 && previous();
      const deleted = await StoryRepository.softDeleteStory(currentStory?.storyId);
      if (deleted) {
        current === 0 && previous();
  ...
  const onPressHyperLink = useCallback(async () => {
    ...
    const hyperlinkUrl = storyHyperLink?.url?.includes('http') ? storyHyperLink?.url : `https://${storyHyperLink?.url}`;
  ...
  return (
    <View style={[styles.container]}>
      <SafeAreaView>
        <View style={styles.backgroundContainer}>
          {currentStory?.dataType === 'video' ? (
            ...
            <Image
              onLoadStart={() => setLoad(true)}
              onError={({ nativeEvent: { error: err } }) => err && setError(true)}
              onLoadEnd={() => start()}
      ...
      <GestureRecognizer onSwipeUp={onPressComment} style={styles.flexCol}>
        ...
        <View style={[styles.userContainer]}>
          <View style={styles.flexRowCenter}>
            <TouchableOpacity
              ...
              hitSlop={10}
            >
              <Image style={[styles.avatarImage]} source={{ uri: communityAvatar }} />
              {hasStoryPermission && (
                <SvgXml
                  height={12}
                  width={12}
            ...
            <View>
              <Text onPress={() => onPressCommunityName && onPressCommunityName()} style={styles.avatarText}>
                {communityData?.displayName}
              </Text>
              <View style={styles.flexRowCenter}>
                <Text style={[styles.avatarSubText, { marginLeft: 10 }]}>{timeDifference} . </Text>
                <Text style={styles.avatarSubText}>By {currentStory?.creator.displayName}</Text>
              </View>
            </View>
          </View>
          <View style={styles.closeIconContainer}>
            <View style={styles.menuCloseContaier}>
              {hasStoryPermission && (
                <TouchableOpacity hitSlop={5} style={styles.threeDotsMenu} onPress={onPressMenu}>
        ...
        {storyHyperLink && (
          <TouchableOpacity style={styles.hyperlinkContainer} onPress={onPressHyperLink}>
            <SvgXml xml={storyHyperLinkIcon('blue')} width="25" height="25" />
            <Text style={styles.hyperlinkText}>{storyHyperLink.customText}</Text>
          </TouchableOpacity>
        )}
        {currentStory?.dataType === 'video' && (
          <TouchableOpacity style={styles.muteBtn} onPress={() => setMuted((prev) => !prev)}>
            <Image source={muted ? muteIcon : unmuteIcon} style={styles.muteIcon} />
          </TouchableOpacity>
        )}
      </GestureRecognizer>
      <View style={styles.footer}>
        {hasStoryImpressionPermission ? (
          <TouchableOpacity style={[styles.seenContainer, { backgroundColor: storyViewerBgColor }]}>
        ...
        <View style={styles.seenContainer}>
          <TouchableOpacity
            style={[styles.iconContainer, { backgroundColor: storyCommentBgColor }]}
          ...
          >
            <SvgXml xml={isLiked ? storyLikedIcon : storyLikeIcon} width="25" height="25" />
