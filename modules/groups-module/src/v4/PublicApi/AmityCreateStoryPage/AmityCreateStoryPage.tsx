...
import { useCameraDevice, Camera, PhotoFile, VideoFile, useCameraFormat } from 'react-native-vision-camera';
...
const AmityCreateStoryPage: FC<ICreateStoryPage> = ({ targetId, targetType, onCreateStory, onDiscardStory }) => {
  ...
  const [activeSwitch, setActiveSwitch] = useState<ACTIVE_SWITCH>(ACTIVE_SWITCH.camera);
  ...
  const [mediaTypeData, setMediaTypeData] = useState<TAmityStoryMediaType>(null);
  const format = Platform.select({
    ios: useCameraFormat(activeCamera, [{ photoAspectRatio: 16 / 9 }, { videoAspectRatio: 16 / 9 }]),
  ...
  const onFinishCapture = useCallback((cameraData: PhotoFile | VideoFile | undefined) => {
    setTotalTime(0);
  ...
  const onStartRecord = useCallback(() => {
    ...
    cameraRef?.current?.startRecording({
      ...
      onRecordingError: (error) => Alert.alert('Video Record Error', error.message),
      fileType: 'mp4',
      flash: flashOnState ? 'on' : 'off',
    });
  }, [flashOnState, onFinishCapture]);
  ...
  const renderCaptureBtn = useCallback(() => {
    if (isCamera) {
      return (
        <Pressable style={styles.cameraCaptureBtn} onPress={onPressCameraCapture}>
          <View style={styles.cameraCaptureBtnFill} />
        </Pressable>
      );
    }
    return (
      <Pressable style={styles.videoCaptureBtn} onPressIn={onStartRecord} onPressOut={onStopRecord}>
        <View style={[styles.videoCaptureInnerButton, !isRecording && { width: 62, height: 62, borderRadius: 100 }]}>
          <Progress.Circle size={75} progress={totalTime / TIMER_LIMIT} borderColor="transparent" color="red" />
        </View>
      </Pressable>
    );
    ...
    <SafeAreaView style={styles.wrapper}>
      <View style={styles.cameraContainer}>
        ...
        <TouchableOpacity style={styles.flashIcon} onPress={onPressFlash}>
          <SvgXml xml={flashOnState ? flashOn() : flashOff()} width={24} height={24} />
        </TouchableOpacity>
      ...
      <View style={styles.switchContainer}>
        <Pressable style={[styles.switch, isCamera && activeSwitchColor]} onPress={onPressCamera}>
          <Text style={styles.switchTxt}>Photo</Text>
        </Pressable>
        <Pressable style={[styles.switch, isVideo && activeSwitchColor]} onPress={onPressVideo}>
          <Text style={styles.switchTxt}>Video</Text>
        </Pressable>
      </View>
      <Modal visible={showDraftStory} animationType="slide" onRequestClose={() => setShowDraftStory(false)}>
        <AmityDraftStoryPage
          targetId={targetId}
