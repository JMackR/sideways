...
const useRequestPermission = async () => {
  ...
  const microphonePermissionresult = !hasMicrophonePermission && (await requestMicrophonePermission());
  !cameraPermissionResult || (!microphonePermissionresult && (await Linking.openSettings()));
  return;
};
