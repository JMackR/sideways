...
export const useGlobalStory = () => {
  const globalStoriesOnNextPageRef = useRef<(() => void) | undefined | null>(null);
  const [fetching, setFetching] = useState(false);
  const [globalStoryTargetsOnNextPage, setGlobalStoryTargetsOnNextPage] = useState<(() => void) | null>(null);
  const [globalStories, setGlobalStories] = useState<Amity.Story[]>([]);
  const [globalStoryTargets, setGlobalStoryTargets] = useState<Amity.StoryTarget[]>([]);
    ...
    const unsubscribe = StoryRepository.getGlobalStoryTargets(
      ...
      ({ data, error, loading, hasNextPage, onNextPage }) => {
        ...
        if (!loading) {
          ...
          setGlobalStoryTargetsOnNextPage(() => (hasNextPage ? onNextPage : null));
        }
      },
    );
    return () => unsubscribe();
  ...
  const getGlobalStories = useCallback(
    (param: { targets: Amity.StoryTargetQueryParam[]; options?: Amity.StorySortOption }) => {
      const unsubscribe = StoryRepository.getStoriesByTargetIds(
        ...
        ({ data, error, loading, hasNextPage, onNextPage }) => {
          ...
          if (!loading) {
            ...
            globalStoriesOnNextPageRef.current = hasNextPage ? onNextPage : null;
          }
        },
      );
      return () => unsubscribe();
