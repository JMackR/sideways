...
const useValidateConfig = (config: IConfigRaw | undefined) => {
  ...
  if (!config.theme || !Object.keys(config.theme).some((item) => ['dark', 'light'].includes(item))) {
    console.error(
  ...
  if (
    ...
    !Object.keys(THEME_COLORS).every(
      (item) =>
        config.theme && Object.keys(config.theme.dark).includes(item) && Object.keys(config.theme.light).includes(item),
    )
  ) {
    console.error(
  ...
  if (!config.excludes || !Array.isArray(config.excludes)) {
    console.error('Config Json Error: `excluded` should be string array. please check `IConfigRaw` Interface');
    return false;
  }
  if (
    !config.customizations ||
    !Object.values(DefaultConfigID).every((item) => Object.keys(config.customizations).includes(item))
  ) {
    console.error(
