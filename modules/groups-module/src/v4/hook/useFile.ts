...
export const useFile = () => {
  const getImage = useCallback(async ({ fileId, imageSize = ImageSizeState.medium }: useFileProps) => {
    if (!fileId) return defaultAvatarUri;
    const file = await FileRepository.getFile(fileId);
    if (!file) return defaultAvatarUri;
    const newImageUrl = FileRepository.fileUrlWithSize(file.data.fileUrl, imageSize) ?? defaultAvatarUri;
    return newImageUrl;
  }, []);
  return { getImage };
};
