...
import { arrowOutlined, blockOrUnblock, editIcon, report, unfollow } from '../../../svg/svg-xml-list';
...
export default function UserProfileSetting({
  ...
  const renderSectionItem = useCallback(
    ({ item }) => {
      return (
        <TouchableOpacity key={item.label} style={styles.rowContainer} onPress={item.callBack}>
          <View style={styles.iconContainer}>
            <SvgXml xml={item.leftIcon()} width="20" height="20" />
          </View>
          <Text style={styles.rowText}>{item.label}</Text>
          {item.rightIcon && <SvgXml xml={item.rightIcon()} width="20" height="20" />}
        </TouchableOpacity>
      );
    },
    [styles],
