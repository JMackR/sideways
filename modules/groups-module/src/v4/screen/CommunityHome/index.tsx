...
import React, { type MutableRefObject, useRef, useState, useCallback, useMemo } from 'react';
import {
  View,
  Image,
  Text,
...
export default function CommunityHome({ route }: any) {
  ...
  const joinCommunityButton = () => {
    return (
      <View style={styles.joinContainer}>
        <TouchableOpacity style={styles.joinCommunityButton} onPress={onJoinCommunityTap}>
        ...
        <View style={styles.pendingPostWrap}>
          <View style={styles.pendingPostArea}>
            ...
            <Text style={styles.pendingDescriptionText}>
              {isUserHasPermission
                ? (pendingPosts.length > 30 && 'More than ') + pendingPosts.length + ' posts need approval'
  ...
  const handleOnPressPostBtn = () => {
    return dispatch(
      openPostTypeChoiceModal({
        ...
        needApprovalOnPostCreation: (communityData?.data as Record<string, any>)?.needApprovalOnPostCreation,
      }),
    );
  };
  ...
  const renderTabs = () => {
    if (currentTab === TabName.Timeline) return <Feed targetType="community" targetId={communityId} ref={feedRef} />;
    if (currentTab === TabName.Gallery)
      return <GalleryComponent targetId={communityId} ref={feedRef} targetType="community" />;
    return null;
  };
      ...
      >
        <View style={styles.imageContainer}>
          ...
          <View style={styles.overlay}>
            <Text style={styles.overlayCommunityText}>{communityData?.data.displayName}</Text>
          </View>
        </View>
        <View style={styles.row}>
          ...
          <View style={styles.rowItemContent}>
            ...
            <TouchableOpacity onPress={() => handleMembersPress()} style={[styles.rowItem, { paddingLeft: 10 }]}>
              <Text style={styles.rowNumber}>{communityData?.data.membersCount}</Text>
              <Text style={styles.rowLabel}>members</Text>
            </TouchableOpacity>
          </View>
        </View>
        <Text style={styles.textComponent}>{communityData?.data.description}</Text>
        {isUserHasPermission && (
        ...
        {!excludes.includes(`*/${ComponentID.StoryTab}/*`) && shouldShowAmityStoryTab() && (
          <AmityStoryTabComponent type={AmityStoryTabComponentEnum.communityFeed} targetId={communityId} />
        )}
        <CustomTab tabName={[TabName.Timeline, TabName.Gallery]} onTabChange={handleTab} />
        {renderTabs()}
      </ScrollView>
      {isJoin && <FloatingButton onPress={handleOnPressPostBtn} isGlobalFeed={false} />}
    </View>
  );
}
