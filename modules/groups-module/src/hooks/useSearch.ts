...
export type TSearchItem = Amity.User & ISearchItem & Amity.Membership<'community'> & { name: string; id: string };

const useSearch = (searchText: string = '', privateCommunityId: string = '') => {
  ...
  const searchAllUsers = useCallback((text: string) => {
    return UserRepository.getUsers(
      ...
      ({ data, error, hasNextPage, onNextPage }) => {
        if (error) return null;
        hasNextPage ? (onNextPageRef.current = onNextPage) : (onNextPageRef.current = null);
        const mappedSearchData = data.map((item) => {
          return {
            ...item,
  ...
  }, [privateCommunityId, searchAllUsers, searchPrivateCommunityMember, searchText]);
