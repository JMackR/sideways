...
export const useTimeDifference = (timestamp: string, isStory?: boolean): string => {
  const timeDifference = useMemo(() => {
    ...
    if (differenceMinutes < 60)
      return `${differenceMinutes}${isStory ? 'm' : differenceMinutes === 1 ? 'min ago' : 'mins ago'}`;
    if (differenceHour < 24)
      return `${differenceHour}${isStory ? 'h' : differenceHour === 1 ? 'hour ago' : 'hours ago'}`;
    if (differenceDay < 365) return `${differenceDay} ${differenceDay === 1 ? 'day ago' : 'days ago'}`;
