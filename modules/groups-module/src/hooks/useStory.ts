import { ReactionRepository, StoryRepository } from '@amityco/ts-sdk-react-native';
...
export const useStory = () => {
  ...
  const handleReaction = useCallback(
    async ({ targetId, reactionName, isLiked }: { targetId: string; reactionName: string; isLiked: boolean }) => {
      try {
        if (isLiked) {
          await ReactionRepository.removeReaction('story', targetId, reactionName);
        } else {
