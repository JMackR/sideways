...
const EditCommentModal = ({ visible, onClose, commentDetail, onFinishEdit }: IModal) => {
  ...
  const handleEditComment = async () => {
    if (inputMessage) {
      const editedComment = await editComment(inputMessage, commentDetail.commentId, 'post');
      if (editedComment) {
        onFinishEdit && onFinishEdit(inputMessage);
      }
    }
  };
  const disabledState = !inputMessage || inputMessage === commentDetail?.data?.text;
  ...
  return (
    <Modal visible={visible} animationType="slide">
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={handleEditComment} style={styles.headerTextContainer} disabled={disabledState}>
