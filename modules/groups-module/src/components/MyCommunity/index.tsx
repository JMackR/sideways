...
export default function MyCommunity() {
  ...
  const queryCommunities = () => {
    const unsubscribe = CommunityRepository.getCommunities({ membership: 'member', limit: 8 }, ({ data }) => {
    ...
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        ...
        <TouchableOpacity onPress={onClickSeeAll}>
          <SvgXml style={styles.arrowIcon} width={17} height={17} xml={arrowOutlined(theme.colors.base)} />
        </TouchableOpacity>
      </View>

      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.scrollView}>
        {communityItems.map((item) => (
          <CommunityList key={item.communityId} item={item} onClickItem={onClickItem} />
        ))}
        <TouchableOpacity onPress={onClickSeeAll} style={styles.seeAllBtn}>
          <View style={styles.seeAllIcon}>
            <SvgXml width={15} height={15} xml={arrowOutlined(theme.colors.base)} />
          </View>
