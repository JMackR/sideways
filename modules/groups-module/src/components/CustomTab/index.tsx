...
import { Animated, type LayoutChangeEvent, type StyleProp, Text, TouchableOpacity, View } from 'react-native';
...
const CustomTab = ({ tabName, onTabChange }: ICustomTab): ReactElement => {
  ...
  const handleTabPress = ({ name, tabIndex }: { name: TabName; tabIndex: number }) => {
    ...
    <View style={styles.container}>
      {tabName.map((tab, index) => {
        ...
        return (
          ...
          >
            <Text style={[styles.tabText, activeTab === index + 1 && styles.activeTabText]}>{tab}</Text>
          </TouchableOpacity>
        );
      })}
      <Animated.View style={[styles.indicator, getIndicatorPosition(), dynamicWidthStyle]} />
    </View>
  );
};
