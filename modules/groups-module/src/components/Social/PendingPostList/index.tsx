...
export default function PendingPostList({ postDetail, onAcceptDecline, isModerator = false }: IPendingPostList) {
  ...
  const { data, postId, createdAt, user, childrenPosts = [], mentionPosition } = postData ?? {};
    ...
    <View key={postId} style={styles.postWrap}>
      ...
      <View>
        <View style={styles.bodySection}>
          ...
          {textPost && <RenderTextWithMention textPost={textPost} mentionPositionArr={mentionPositionArr} />}
          {childrenPosts.length > 0 && <View style={styles.mediaWrap}>{!loading && memoizedMediaSection}</View>}
        </View>
        {isModerator && (
          <View style={styles.actionSection}>
            <TouchableOpacity onPress={() => approvePost()} style={styles.acceptBtn}>
              <Text style={styles.acceptBtnText}>Accept</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => declinePost()} style={styles.declineBtn}>
