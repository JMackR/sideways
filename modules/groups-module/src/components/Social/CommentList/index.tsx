...
import { expandIcon, likeCircle, likedXml, likeXml, personXml, replyIcon, threeDots } from '../../../svg/svg-xml-list';
...
const CommentList = ({ commentDetail, onDelete, onClickReply }: ICommentList) => {
  ...
  const [isLike, setIsLike] = useState<boolean>(myReactions ? myReactions.includes('like') : false);
  const [likeReaction, setLikeReaction] = useState<number>(reactions?.like ? reactions?.like : 0);
      ...
      <View style={styles.headerSection}>
        ...
        <View style={styles.rightSection}>
          ...
          <View style={styles.timeRow}>
            ...
            {(editedAt !== createdAt || isEditComment) && <Text style={styles.headerTextTime}>Edited</Text>}
          </View>
          <View style={styles.commentBubble}>
            {textComment && <RenderTextWithMention mentionPositionArr={mentionPosition} textPost={textComment} />}
            {/* <Text style={styles.commentText}>{textComment}</Text> */}
          </View>
          <View style={styles.actionSection}>
            <TouchableOpacity onPress={() => addReactionToComment()} style={styles.likeBtn}>
              {isLike ? (
              ...
              <Text style={isLike ? styles.likedText : styles.btnText}>{!isLike ? 'Like' : 'Liked'}</Text>
            </TouchableOpacity>
            {likeReaction > 0 && (
              <TouchableOpacity onPress={onPressCommentReaction} style={styles.likeBtn}>
          ...
          {isOpenReply && (
            <FlatList
              ...
              renderItem={({ item }) => (
                <ReplyCommentList commentId={item.commentId} commentDetail={item} onDelete={onDelete} />
              )}
          ...
          {childrenComment.length > 0 && !isOpenReply && (
            <TouchableOpacity onPress={() => openReplyComment()} style={styles.viewMoreReplyBtn}>
          ...
          {isOpenReply && hasNextPage && (
            <TouchableOpacity onPress={() => onNextPage()} style={styles.viewMoreReplyBtn}>
              <SvgXml xml={expandIcon} />
        ...
        <Pressable onPress={closeModal} style={styles.modalContainer}>
          <Animated.View
            style={[
              styles.modalContent,
              modalStyle,
              user?.userId === (client as Amity.Client).userId && styles.twoOptions,
            ]}
          >
            {user?.userId === (client as Amity.Client).userId ? (
              <View>
                <TouchableOpacity onPress={openEditCommentModal} style={styles.modalRow}>
                  <Text style={styles.deleteText}> Edit Comment</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={deletePostObject} style={styles.modalRow}>
                  <Text style={styles.deleteText}> Delete Comment</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <TouchableOpacity onPress={reportCommentObject} style={styles.modalRow}>
                <Text style={styles.deleteText}>{isReportByMe ? 'Undo Report' : 'Report'}</Text>
              </TouchableOpacity>
            )}
          </Animated.View>
        </Pressable>
