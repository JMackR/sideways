...
export default function PostList({ postDetail, postIndex, onDelete, isGlobalfeed }: IPostList) {
  ...
  const renderLikeText = useCallback((likeNumber: number | undefined): string => {
    if (!likeNumber) return '';
    if (likeNumber === 1) return 'like';
    return 'likes';
  }, []);
  const renderCommentText = useCallback((commentNumber: number | undefined): string => {
  ...
  async function getCommunityInfo(id: string) {
    const { data: community }: { data: Amity.LiveObject<Amity.Community> } = await getCommunityById(id);
        ...
        <Pressable onPress={closeModal} style={styles.modalContainer}>
          ...
          >
            {user?.userId === (client as Amity.Client).userId ? (
              <View>
                <TouchableOpacity onPress={openEditPostModal} style={styles.modalRow}>
              ...
              <TouchableOpacity onPress={reportPostObject} style={styles.modalRow}>
    ...
    <View key={postId} style={styles.postWrap}>
        ...
        <View style={styles.user}>
          ...
          <View style={styles.fillSpace}>
            <View style={styles.headerRow}>
              ...
              {communityName && (
                <View style={styles.communityNameContainer}>
                  <SvgXml style={styles.arrow} xml={arrowXml} width="8" height="8" />

                  <TouchableOpacity onPress={handleCommunityNamePress}>
                    <Text ellipsizeMode="tail" numberOfLines={3} style={styles.headerText}>
                      {communityName}
                    </Text>
                  </TouchableOpacity>
                </View>
            ...
            <View style={styles.timeRow}>
              ...
              {(editedAt !== createdAt || isEdit) && <Text style={styles.headerTextTime}>Edited</Text>}
            </View>
          </View>
        </View>
      ...
      <View>
        <View style={styles.bodySection}>
          {textPost && <RenderTextWithMention mentionPositionArr={[...mentionPositionArr]} textPost={textPost} />}
          {childrenPosts?.length > 0 && <MediaSection childrenPosts={childrenPosts} />}
        </View>
            ...
            <View style={styles.countSection}>
              ...
              {commentsCount > 0 && (
                <Text style={styles.commentCountText}>
                  {commentsCount > 0 && commentsCount} {renderCommentText(commentsCount)}
                </Text>
              )}
            </View>
          </View>
        ...
        {targetType !== 'community' || isJoined ? (
          <View style={styles.actionSection}>
            <TouchableOpacity onPress={() => addReactionToPost(isLike)} style={styles.likeBtn}>
              {isLike ? (
          ...
          <View style={styles.actionSection}>
            <Text style={styles.btnText}>Join community to interact with all posts</Text>
          </View>
        )}
      </View>
      {renderOptionModal()}
