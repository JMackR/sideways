...
const PostTypeChoiceModal = () => {
  ...
  const onChooseType = (type: string) => {
    if (targetId && targetName && targetType) {
      const targetscreen = type === 'post' ? 'CreatePost' : type === 'poll' ? 'CreatePoll' : null;
  ...
  return (
    <Modal animationType="fade" transparent={true} visible={showPostTypeChoiceModal} onRequestClose={closeModal}>
      <Pressable onPress={closeModal} style={styles.modalContainer}>
        <Animated.View style={[styles.modalContent, modalStyle]}>
          <TouchableOpacity onPress={() => onChooseType('post')} style={styles.modalRow}>
            <SvgXml xml={postIconOutlined(theme.colors.base)} width="28" height="28" />
            <Text style={styles.postText}>Post</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => onChooseType('poll')} style={styles.modalRow}>
