...
const PollSection: React.FC<IPollSection> = ({ pollId }) => {
  ...
  const { data: pollData, endDays, totalVote, isPollClosed, isAlreadyVoted } = usePoll(pollId, shouldFetch);
    ...
    return (
      <View style={styles.container}>
        <View style={styles.rowContainer}>
          <Text style={styles.pollEndDays}>{isPollClosed ? 'Final result' : `Poll ends in ${endDays}`}</Text>
