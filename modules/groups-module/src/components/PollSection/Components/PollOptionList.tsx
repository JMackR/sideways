...
const PollOptionList: FC<IPollOptionList> = ({
  ...
  const renderViewAllOptions = useMemo(() => {
    const resultText = isPollClosed || isAlreadyVoted ? `View full results` : `${hiddenOptions} more options`;
    return (
      <TouchableOpacity onPress={onPressMoreOption} style={styles.moreOptionsBtn}>
        <Text>{resultText}</Text>
      </TouchableOpacity>
    );
  }, [hiddenOptions, isPollClosed, isAlreadyVoted, onPressMoreOption, styles.moreOptionsBtn]);
  ...
  return (
    <>
      {options.map((option) => {
        ...
        const selectedOptionContainerStyle: ViewStyle = selectedItem && styles.selectedOptionContainer;
        ...
        const onResultOptionStyle: ViewStyle =
          (isAlreadyVoted || isPollClosed) && option.isVotedByUser && styles.onResultOption;
        return (
          <TouchableOpacity
            disabled={isPollClosed || isAlreadyVoted}
            key={`${option.id}${option.isVotedByUser}`}
            style={[styles.pollOptionContainer, selectedOptionContainerStyle, onResultOptionStyle]}
            onPress={() => onPressOption(option.id)}
          >
            <View style={styles.pollOptionContainerRow}>
              {!isPollClosed && !isAlreadyVoted && <SvgXml xml={selectedIcon} width="20" height="20" />}
