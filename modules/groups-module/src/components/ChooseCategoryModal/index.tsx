...
const ChooseCategoryModal = ({ visible, onClose, onSelect, categoryId }: IModal) => {
        ...
        const unsubscribe = CategoryRepository.getCategories(
          ...
          (data: Amity.LiveCollection<Amity.Category>) => {
            if (data) {
              setCategories(data);
              if (categoryId) {
                const currentCategoryName = data.data.find((item) => item.categoryId === categoryId)?.name ?? '';
  ...
  return (
    <Modal visible={visible} animationType="slide">
      <View style={styles.container}>
        ...
        <FlatList
          data={categoriesList}
          renderItem={({ item }) => <Categories item={item} onSelectCategory={onSelectCategory} />}
