...
const AddMembersModal = ({ visible, onClose, onSelect, initUserList, excludeUserList = [] }: IModal) => {
  const styles = useStyle();
  const theme = useTheme() as MyMD3Theme;
  const [sectionedGroupUserList, setSectionedGroupUserList] = useState<SelectUserList[]>(null);
  ...
  const renderSectionHeader = ({ section }: { section: SelectUserList }) => <SectionHeader title={section.title} />;
  ...
  const renderItem = ({ item }: ListRenderItemInfo<UserInterface>) => {
    ...
    return <UserItem showThreeDot={false} user={userObj} isCheckmark={selectedUser} onPress={onUserPressed} />;
  };
    ...
    <Modal visible={visible} animationType="slide">
      <View style={styles.container}>
        ...
        {selectedUserList.length > 0 ? (
          <SelectedUserHorizontal users={selectedUserList} onDeleteUserPressed={onDeleteUserPressed} />
        ) : (
          <View />
        )}
