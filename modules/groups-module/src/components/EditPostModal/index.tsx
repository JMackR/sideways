...
const EditPostModal = ({ visible, onClose, postDetail, onFinishEdit, privateCommunityId }: IModal) => {
  ...
  const parsePostText = useCallback((text: string, mentionUsersArr: TSearchItem[]) => {
  ...
  const getMentionPositions = useCallback((text: string, mentioneeIds: string[]) => {
    let index = 0;
    let mentions = [];
    let match;
  ...
  const handleOnCloseVideo = (originalPath: string, fileId: string) => {
    ...
    setVideoPostList((prevData) => {
      const newData = prevData.filter((item: IVideoPost) => item.videoFileId.original !== fileId); // Filter out objects containing the desired value
      return newData; // Remove the element at the specified index
    });
  };
