...
export function getCommunityById(communityId: string): Promise<any> {
  const communityObject = new Promise((resolve, reject) => {
    let object;
    const unsubscribe = CommunityRepository.getCommunity(communityId, (data) => {
      if (data.error) {
        reject(data.error);
      }
      object = data;
...
export const updateCommunity = (
  ...
  const communityObject = new Promise(async (resolve, reject) => {
    try {
      ...
      const { data: community } = await CommunityRepository.updateCommunity(communityId, newCommunity);
      resolve(community);
    } catch (error) {
      reject(error);
    }
...
export const updateCommunityMember = ({
  ...
  const communityMembers = new Promise<boolean>(async (resolve, reject) => {
    try {
      const updateMembers =
        operation === 'ADD'
          ? await CommunityRepository.Membership.addMembers(communityId, memberIds)
          : await CommunityRepository.Membership.removeMembers(communityId, memberIds);
...
export const assignRolesToUsers = (communityId: string, roles: string[], userIds: string[]) => {
  const assignUserRoles = new Promise<boolean>(async (resolve, reject) => {
    try {
      const result = await CommunityRepository.Moderation.addRoles(communityId, roles, userIds);
      resolve(result);
    } catch (error) {
      reject(error);
    }
...
export const removeRolesFromUsers = (communityId: string, roles: string[], userIds: string[]) => {
  const removeUserRoles = new Promise<boolean>(async (resolve, reject) => {
    try {
      const result = await CommunityRepository.Moderation.removeRoles(communityId, roles, userIds);
      resolve(result);
    } catch (error) {
      reject(error);
    }
