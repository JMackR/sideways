...
export async function addCommentReaction(commentId: string, reactionName: string): Promise<boolean> {
  const reactionObject: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
      const isCommentReactionAdded = await ReactionRepository.addReaction('comment', commentId, reactionName);
...
export async function removeCommentReaction(commentId: string, reactionName: string): Promise<boolean> {
  const reactionObject: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
      const isCommentReactionRemoved = await ReactionRepository.removeReaction('comment', commentId, reactionName);
...
export async function createComment(
  ...
  const createCommentObject: Promise<Amity.InternalComment> = new Promise(async (resolve, reject) => {
    try {
      const newComment = {
        data: {
          text: text,
...
export async function getCommentsDataByIds(commentIds: string[]): Promise<Amity.InternalComment[]> {
  const commentObject: Promise<Amity.InternalComment[]> = new Promise(async (resolve, reject) => {
    try {
...
export async function deleteCommentById(commentId: string): Promise<boolean> {
  const isDeletedObject: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
      const hardDelete = await CommentRepository.deleteComment(commentId, true);
      if (hardDelete) {
        resolve(true);
      }
    } catch (error) {
