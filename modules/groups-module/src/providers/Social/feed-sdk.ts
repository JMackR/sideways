...
export async function getGlobalFeed(page: Amity.Page<number>): Promise<IGlobalFeedRes> {
  const feedObject: Promise<IGlobalFeedRes> = new Promise(async (resolve, reject) => {
    try {
      const { data, nextPage, prevPage } = await FeedRepository.queryGlobalFeed({
        page,
      });
...
export async function addPostReaction(postId: string, reactionName: string): Promise<boolean> {
  const reactionObject: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
...
export async function removePostReaction(postId: string, reactionName: string): Promise<boolean> {
  const reactionObject: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
...
export function getPostById(postId: string): Promise<any> {
  const communityObject = new Promise((resolve, reject) => {
    let object;
    const unsubscribe = PostRepository.getPost(postId, ({ data: postInfo, loading, error }) => {
      if (error) {
        reject(error);
      }
      if (!loading) {
...
export async function createPostToFeed(
  ...
  if (postType === 'text') {
    ...
    const formattedFileIds: { type: string; fileId: string }[] = content.fileIds.map((id) => {
  ...
  const createPostObject: Promise<Amity.Post<any>> = new Promise(async (resolve, reject) => {
    try {
...
export async function editPost(
  ...
  const editPostObject: Promise<Amity.Post<any>> = new Promise(async (resolve, reject) => {
    try {
...
export async function deletePostById(postId: string): Promise<boolean> {
  const isDeletedObject: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
...
export async function reportTargetById(targetType: 'post' | 'comment', postId: string): Promise<boolean> {
...
export async function isReportTarget(targetType: 'post' | 'comment', targetId: string): Promise<boolean> {
  const isReport: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
...
export async function unReportTargetById(targetType: 'post' | 'comment', targetId: string): Promise<boolean> {
  const isReport: Promise<boolean> = new Promise(async (resolve, reject) => {
    try {
