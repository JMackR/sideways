...
import { IConfigRaw, IUIKitConfig, IUIKitConfigOptions } from '~/v4/types/config.interface';
...
export const ConfigProvider = ({ children, configs }: IConfigProviderProps) => {
  ...
  const getPossibleIDs = ({ page, component, element }: IUIKitConfigOptions): string[] => {
    if (!component && !element) return [`${page}/*/*`];
    if (!element) return [`${page}/${component}/*`, `*/${component}/*`];
    return [`${page}/${component}/${element}`, `${page}/*/${element}`, `*/${component}/${element}`, `*/*/${element}`];
  };
  ...
  const getUiKitConfig = ({ page, component, element }: IUIKitConfigOptions) => {
  ...
  return (
    <ConfigContext.Provider value={{ globalTheme, excludes, getConfig, getUiKitConfig }}>
      {children}
