...
export const EditProfile: React.FC<EditProfileProps> = ({ navigation, route }) => {
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      ...
      title: 'Edit Profile',
      headerRight: () => <DoneButton navigation={navigation} onDonePressed={onDonePressed} buttonTxt="Save" />,
    });
  }, [navigation]);
  ...
  const onDonePressed = async () => {
    try {
      ...
      const userDetail: Partial<
        Pick<Amity.User, 'displayName' | 'avatarFileId' | 'avatarCustomUrl' | 'description' | 'metadata'>
      > = {
        displayName: displayNameRef.current,
        description: aboutRef.current ?? '',
      };
      const file: Amity.File<any>[] = imageUriRef.current && (await uploadImageFile(imageUriRef.current));
      if (file) {
  ...
  return (
    <View style={styles.container}>
      <LoadingOverlay isLoading={showLoadingIndicator} loadingText="Loading..." />
      <View style={styles.avatarContainer}>
        <TouchableOpacity onPress={handleAvatarPress}>
          <Image
            style={styles.avatar}
            source={imageUri ? { uri: imageUri } : require('../../../assets/icon/Placeholder.png')}
          />
        </TouchableOpacity>
        <View style={styles.cameraIconContainer}>
          <TouchableOpacity onPress={handleAvatarPress}>
            <View style={styles.cameraIcon}>
              <Image source={require('../../../assets/icon/cameraIcon.png')} style={styles.imageIcon} />
            </View>
          </TouchableOpacity>
        </View>
      </View>
      <View style={styles.displayNameContainer}>
        ...
        <View style={styles.characterCountContainer}>
          <Text style={styles.characterCountText}>{`${displayNameCharacterCount}/${MAX_CHARACTER_COUNT}`}</Text>
        </View>
      </View>
      ...
      <View style={styles.displayNameContainer}>
        ...
        <View style={styles.characterCountContainer}>
          <Text style={styles.characterCountText}>{`${aboutCharacterCount}/${MAX_CHARACTER_COUNT}`}</Text>
        </View>
      </View>
      <TextInput
        style={styles.input}
