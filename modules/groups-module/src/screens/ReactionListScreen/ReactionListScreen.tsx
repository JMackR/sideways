...
const ReactionListScreen = ({ navigation, route }) => {
  ...
  const renderReactors = useCallback(
    ({ item }: { item: Amity.User }) => {
      ...
      return (
        <TouchableOpacity style={styles.rowContainer} onPress={() => onPressReactor(item.userId)}>
  ...
  const errorContent = useCallback(() => {
    return (
      <View style={styles.errorContainer}>
        ...
        <Text style={styles.errorTitle}>Sorry, we couldn't show the content</Text>
        <Text style={styles.errorDesc}>Please try again or contact the administator{'\n'}for more information</Text>
      </View>
    );
      ...
      <>
        <View style={styles.header}>
          <View style={styles.reactionCountContainer}>
            <Text style={styles.reactionCount}>All {formatNumber(reactions?.all)}</Text>
          </View>
        </View>
        <FlatList data={reactors} renderItem={renderReactors} keyExtractor={(item) => item.userId} />
      </>
    );
  ...
  return (
    <View style={styles.container}>
      {loading ? renderContentLoading() : isError ? errorContent() : renderContent()}
    </View>
  );
};
