...
const PostDetail = () => {
  ...
  const privateCommunityId = !communityObject?.isPublic && communityObject?.communityId;
  ...
  const handleSend: () => Promise<void> = async () => {
    ...
    if (isFromGlobalfeed) {
      dispatch(updateByPostIdGlobalFeed({ postId: postId, postDetail: updatedPost }));
    } else {
  ...
  const onDeleteComment = async (commentId: string) => {
    ...
    if (isDeleted) {
      ...
      const updatedCommentList: IComment[] = prevCommentList.filter((item) => item.commentId !== commentId);
  ...
  return loading ? (
    ...
    >
      <ScrollView onScroll={handleScroll} style={styles.container}>
        <PostList onChange={() => {}} postDetail={currentPostdetail as IPost} isGlobalfeed={isFromGlobalfeed} />
      ...
      {replyUserName.length > 0 && (
        <View style={styles.replyLabelWrap}>
          <Text style={styles.replyLabel}>
            Replying to <Text style={styles.userNameLabel}>{replyUserName}</Text>
          </Text>
          <TouchableOpacity>
      ...
      <View style={styles.InputWrap}>
        ...
        <TouchableOpacity disabled={inputMessage.length > 0 ? false : true} onPress={handleSend} style={styles.postBtn}>
          <Text style={inputMessage.length > 0 ? styles.postBtnText : styles.postDisabledBtn}>Post</Text>
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
