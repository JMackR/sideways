...
export default function UserProfile({ route }: any) {
  ...
  const galleryRef: MutableRefObject<FeedRefType | null> = useRef<FeedRefType | null>(null);
  ...
  const followButton = () => {
    return (
      <TouchableOpacity style={styles.followButton} onPress={onFollowTap}>
        <Image source={require('../../../assets/icon/followPlus.png')} style={styles.followIcon} />
  ...
  const renderTabs = () => {
    if (currentTab === TabName.Timeline) return <Feed targetType="user" targetId={userId} ref={feedRef} />;
    if (currentTab === TabName.Gallery) return <UserProfileGallery userId={userId} ref={galleryRef} />;
    return null;
  };

  return (
    <View style={styles.container}>
      <ScrollView style={{ flex: 1 }} ref={scrollViewRef} onScroll={handleScroll} scrollEventThrottle={20}>
          ...
          <View style={styles.userDetail}>
            ...
            <View style={styles.userInfo}>
              ...
              <View style={styles.horizontalText}>
                <Text style={styles.textComponent}>{followingCount + ' Following '}</Text>
                <Text style={styles.textComponent}>{followerCount + ' Follower'}</Text>
              </View>
            </View>
          </View>
          <View style={styles.descriptionContainer}>
            {user?.description ? <Text style={styles.descriptionText}> {user?.description}</Text> : <View />}
          </View>
          {renderButtons()}
        </View>
        {!isBlocked && (
          <>
            <CustomTab tabName={[TabName.Timeline, TabName.Gallery]} onTabChange={setCurrentTab} />
            {renderTabs()}
          </>
        )}
      </ScrollView>
