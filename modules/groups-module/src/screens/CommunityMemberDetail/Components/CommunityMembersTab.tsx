...
const CommunityMembersTab: React.FC<ICommunityMembersTab> = ({
  ...
  const renderMember = ({ item }: { item: Amity.Membership<'community'> }) => {
    if ((item as Record<string, any>).user) {
      ...
      return <UserItem user={userObject} showThreeDot={true} onThreeDotTap={onThreeDotTap} hideMenu={isOwner} />;
    }
    return null;
  };
  ...
  return (
    <FlatList
      refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['lightblue']} tintColor="lightblue" />
      }
      data={members}
