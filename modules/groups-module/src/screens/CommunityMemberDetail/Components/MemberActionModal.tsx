import { Animated, Modal, Pressable, Text, TextStyle, TouchableOpacity } from 'react-native';
...
const MemberActionModal: FC<IMemberActionModal> = ({
  ...
  const actionData = useMemo(
    ...
    [communityId, currentUserId, hasModeratorPermission, isInModeratorTab, userId],
  );
    ...
    <Modal animationType="fade" transparent={true} visible={isVisible} onRequestClose={closeModal}>
      <Pressable onPress={closeModal} style={styles.modalContainer}>
        <Animated.View style={[styles.modalContent, modalStyle]}>
          {actionData.map((data) => {
            const warningStyle: TextStyle = data.id === 'remove' ? { color: 'red' } : null;
            if (data.shouldShow) {
              return (
                <TouchableOpacity key={data.id} onPress={() => onPressAction(data)} style={styles.modalRow}>
                  <Text style={[styles.postText, warningStyle]}>{data.label}</Text>
                </TouchableOpacity>
              );
            } else return null;
          })}
