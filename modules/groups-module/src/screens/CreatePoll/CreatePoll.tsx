...
const CreatePoll = ({ navigation, route }) => {
  ...
  const [pollOptions, setPollOptions] = useState<Pick<Amity.PollAnswer, 'data' | 'dataType'>[]>([]);
  ...
  const [timeFrame, setTimeFrame] = useState<{ key: number; label: string }>(null);
  const { targetId, targetType, targetName, postSetting, needApprovalOnPostCreation, isPublic } = route.params;
  ...
  const handleCreatePost = useCallback(async () => {
    ...
    try {
      ...
      const res = await checkCommunityPermission(targetId, client as Amity.Client, apiRegion);
  ...
  return (
    <View style={styles.AllInputWrap}>
      <Header targetName={targetName} goBack={goBack} isBtnDisable={isBtnDisable} handleCreatePost={handleCreatePost} />
      <ScrollView
        scrollEnabled={isScrollEnabled}
        contentContainerStyle={styles.scrollContainer}
        keyboardShouldPersistTaps="handled"
      >
        {loading && <ActivityIndicator animating={loading} color={'black'} size="large" />}
        <View style={styles.inputContainer}>
          <View style={styles.rowContainer}>
            ...
            <Text style={styles.maxPollQuestionText}>{`${optionQuestion.length}/${MAX_POLL_QUESRION_LENGTH}`}</Text>
          </View>
        ...
        <View style={styles.inputContainer}>
          <View style={styles.rowContainer}>
            ...
            <Text style={styles.maxPollQuestionText}>{`${pollOptions.length}/${MAX_OPTIONS}`}</Text>
          </View>
          <Text style={styles.subtitle}>Choose at least {MIN_OPTIONS} options</Text>
          {pollOptions.map((pollOption, index) => {
            ...
            return (
              <>
                <View key={index} style={[styles.pollOptionContainer, errorContinerStyle]}>
                  <View style={styles.rowContainer}>
                    <TextInput
                    ...
                    />
                    <SvgXml xml={circleCloseIcon} width="20" height="20" onPress={() => onPressRemoveOption(index)} />
                  </View>
                </View>
                {onReachMaxChar && <Text style={styles.errorText}>You have reached 200 character limit</Text>}
              </>
            );
          })}
          {pollOptions.length < 10 && (
            <TouchableOpacity style={styles.addOptionBtn} onPress={onPressAddOption}>
              <SvgXml xml={plusIcon(theme.colors.base)} width="20" height="20" />
        ...
        <View style={styles.inputContainer}>
          <View style={styles.rowContainer}>
            <View style={styles.fillSpace}>
              ...
              <Text style={styles.subtitle}>Turn on to allow others to vote more than one option</Text>
            </View>
            <Switch value={isMultipleOption} onValueChange={setIsMultipleOption} />
          </View>
        </View>
        <View style={styles.inputContainer}>
          <Text style={styles.inputTitle}>Schedule poll (Optional)</Text>
          <Text style={styles.subtitle}>Poll will be automatically closed at the end of chosen time frame (UTC)</Text>
          <ModalSelector
            data={data}
