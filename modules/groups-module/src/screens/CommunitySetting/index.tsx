...
import { View, Text, TouchableOpacity, Image, SectionList, Alert } from 'react-native';
...
export const CommunitySetting: React.FC<ChatDetailProps> = ({ navigation, route }) => {
  ...
  const onLeaveCommunity = async () => {
    try {
      ...
      if (
        ...
        error.message.includes("Can't leave the community because you are the only active moderator in this community")
      ) {
        Alert.alert(
        ...
        console.error("Error: Can't leave the community due to being the only active moderator");
      } else {
        Alert.alert('Unable to leave community', 'Something went wrong. Please try again later');
      }
    }
  };
