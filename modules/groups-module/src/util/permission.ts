...
export const isCommunityModerator = async ({ userId, communityId }: { userId: string; communityId: string }) => {
  if (!userId || !communityId) return false;
  try {
    const { error, loading, data }: Amity.LiveCollection<Amity.Membership<'community'>> = await new Promise(
      (resolve) => {
